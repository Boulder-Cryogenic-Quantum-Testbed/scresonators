...
powers = [-5]
n_avgs = 10
file_names = [f"{sample_name}_{fc-span/2*1e-3:0.3f}GHz_{fc+span/2*1e-3:0.3f}GHz_{powers[0]:.02f}dB".replace(".","p") for fc, span in zip(fcs, spans)]
data_dir = [f"{sample_name}_{fc-span/2*1e-3:0.3f}GHz_{fc+span/2*1e-3:0.3f}GHz" for fc, span in zip(fcs, spans)]
