...
# %% high power scan
high_powers_floats = np.arange(-3, -33, -4)  # about 70 secs/power
high_powers = [int(x) for x in high_powers_floats]
high_powers_plus_atten = [x+ext_atten for x in high_powers]
temperatures = temperature*len(high_powers)
HPow_num_avgs = 10
HPow_IFBW_kHz = 1.0
HPow_num_pts = 51
HPow_spans = [15]*len(fcs)
HPow_fnames = make_filenames(high_powers_plus_atten, freq_strs, temperatures, sample_name, seg_str, tag="")
HPow_fnames = np.array(HPow_fnames).flatten()
HPow_elapsed_time = []

tStart_high = time.time()
measure_multiple_resonators(fcs, HPow_spans, delays, high_powers,
                             ...
                             Noffres = 30, Nf = 150, offresfraction = 0.8, 
                             is_segmented = is_segmented,  bypass_janis = True,
                             segment_option = segment_option, seg_str=seg_str,)
HPow_elapsed_time = time.time() - tStart_high

print_text_block(tStart_high, HPow_num_avgs, HPow_num_pts, HPow_IFBW_kHz, num_powers=len(high_powers))
visualize_data(HPow_fnames, data_dirs, fcs, HPow_spans, high_powers, add_zero_lines=False, filetype='png', show_plots=True, plot_dir='.\\plots\\')

display(time.strftime("High Powers Finished:  [%a, %b %d, %H:%M %p]" + f"\n   {HPow_elapsed_time/60:1.2f} mins"))

# %% mid power scan
med_powers_floats = np.arange(-33, -48, -4)  # 500 secs per power
med_powers = [int(x) for x in med_powers_floats]
med_powers_plus_atten = [x+ext_atten for x in med_powers]
temperatures = temperature*len(med_powers)
MPow_num_avgs = 600
MPow_IFBW_kHz = 0.5
MPow_num_pts = 51
MPow_spans = [15]*len(fcs)
MPow_fnames = make_filenames(med_powers_plus_atten, freq_strs, temperatures, sample_name, seg_str, tag="")
MPow_fnames = np.array(MPow_fnames).flatten()
MPow_elapsed_time = []

tStart_med = time.time()
measure_multiple_resonators(fcs, MPow_spans, delays, med_powers,
                            ...
                            is_segmented = is_segmented,  bypass_janis = True, 
                            segment_option = segment_option, seg_str=seg_str,)
MPow_elapsed_time = time.time() - tStart_med

print_text_block(tStart_med, MPow_num_avgs, MPow_num_pts, MPow_IFBW_kHz, num_powers=len(med_powers))
visualize_data(MPow_fnames, data_dirs, fcs, MPow_spans, med_powers, add_zero_lines=False, filetype='png', show_plots=True, plot_dir='.\\plots\\')

display(time.strftime("Med Powers Finished:  [%a, %b %d, %H:%M %p]") + f"\n   {MPow_elapsed_time/60:1.2f} mins")
...
low_powers_floats = np.arange(-68, -78, -5)  # was about 1 hr/power, now should be ~18 mins
low_powers = [int(x) for x in low_powers_floats]
low_powers_plus_atten = [x+ext_atten for x in low_powers]
temperatures = temperature*len(low_powers)
LPow_num_avgs = 300  
LPow_IFBW_kHz = 0.1  
LPow_num_pts = 301
LPow_spans = [20]*len(fcs)
LPow_fnames = make_filenames(low_powers_plus_atten, freq_strs, temperatures, sample_name, seg_str, tag="")
LPow_fnames = np.array(LPow_fnames).flatten()
LPow_elapsed_time = []
LPow_segment_option = "hybrid"
LPow_seg_str = LPow_segment_option if LPow_segment_option != 'None' else ""
 
tStart_low = time.time()
measure_multiple_resonators(fcs, LPow_spans, delays, low_powers,
                            ...
                            Noffres = 30, Nf = 150, offresfraction = 0.8, 
                            is_segmented = is_segmented,  bypass_janis = True, 
                            segment_option = LPow_segment_option, seg_str=LPow_seg_str,)
LPow_elapsed_time = time.time() - tStart_low

print_text_block(tStart_low, LPow_num_avgs, LPow_num_pts, LPow_IFBW_kHz, num_powers=len(low_powers))
visualize_data(LPow_fnames, data_dirs, fcs, LPow_spans, low_powers, add_zero_lines=False, filetype='png', show_plots=True, plot_dir='.\\plots\\')

display(time.strftime("Med Powers Finished:  [%a, %b %d, %H:%M %p]\n") + f"\n   {LPow_elapsed_time/60:1.2f} mins")

# %% time analysis
# N_res = 7 #len(fcs)

# T_A = 60 #HPow_elapsed_time/N_res
# T_B = 100
# T_C = 10*T_B #LPow_elapsed_timeN_res  
