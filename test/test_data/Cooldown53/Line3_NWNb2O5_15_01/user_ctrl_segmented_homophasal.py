...
# fcs = [ 5.7572, 5.7967, 5.850875, 5.889750,
    #    6.289625, 6.334725, 6.38822, 6.447225]
fcs = [ 6.388225 ]
# ,  
# 
...
jctrl_dict = {"vna_addr": 'TCPIP0::169.254.89.124::inst0::INSTR'}
bypass_janis = True
...
# %% high power scan
high_powers_floats = np.arange(-5, -15, -3)  # about 100 secs/power
...
measure_multiple_resonators(fcs, spans, delays, high_powers,
                             ...
                             is_segmented = is_segmented,  bypass_janis = bypass_janis,
                             segment_option = segment_option, seg_str=seg_str,
                             Jctrl_dict=jctrl_dict)
...
# %% mid power scan
med_powers_floats = np.arange(-40, -60, -4)  # 168 secs per power
...
# low_powers_floats = np.arange(-60, -70, -5)  # 927 secs/power
...
## 5/27 10pm, ran the commented code above, then ran again for below to do two sweeps
## overnight :)

low_powers_floats = np.arange(-70, -80, -5)  # 328 secs/power
low_powers = [int(x) for x in low_powers_floats]
temperatures = temperature*len(low_powers)
LPow_num_avgs = 3000
LPow_IFBW_kHz = 0.1
...
T_A = 100 #HPow_elapsed_time[0]/N_res
T_B = 168
T_C = 331 #LPow_elapsed_time[0]/N_res  

N_A = len(high_powers);  N_B = len(med_powers);  N_C = len(low_powers)
T_A_tot = T_A*N_A;       T_B_tot = T_B*N_B;      T_C_tot = T_C*N_C

T_single_res = (T_A_tot+T_B_tot+T_C_tot)/60
T_all_res = T_single_res * N_res

print(f"Measurement time analysis")
print(f" T_single_res =  ({N_A} high powers * {T_A:1.2f} secs/ea)")
print(f"                + ({N_B} med powers * {T_B:1.2f} secs/ea)")
print(f"                + ({N_C} low powers * {T_C:1.2f} secs/ea) ")
print(f"              = {T_single_res:1.2f} mins/res\n")
print(f" T_all_res  = ({T_A_tot*N_res/60:1.2f} mins for {N_A} hpow of {N_res} resonators)")
print(f"               + ({T_B_tot*N_res/60:1.2f} mins for {N_B} mpow of {N_res} resonators)")
print(f"               + ({T_C_tot*N_res/60:1.2f} mins for {N_C} lpow of {N_res} resonators)")
print(f"               = {T_all_res:1.2f} mins\n")
print(f" T_single_res  = ({T_A_tot/60:1.2f} mins + {T_B_tot/60:1.2f} mins + {T_C_tot/60:1.2f} mins) = {T_single_res:1.2f} mins * {N_res} resonators")
print(f" T_single_res * {N_res} resonators = {T_single_res*N_res:1.2f} mins ({T_all_res/60:1.2f} hrs)")

print(time.strftime("%a, %b %d, %H:%M %p"))
elapsed_time_1 = (time.time() - tStart_high)/60
print(f"\ntime.time() reported elapsed time: {elapsed_time_1:1.2f} mins ({elapsed_time_1/60:1.2f} hrs)")

# %%
elapsed_time_2 = sum([*LPow_elapsed_time, *MPow_elapsed_time, *HPow_elapsed_time])/60
print(f"estimated elapsed time: {T_single_res:1.2f} mins")
print(f"actual elapsed time: {elapsed_time_2:1.2f} mins")
