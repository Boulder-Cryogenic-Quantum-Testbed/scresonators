...
def name_plot(filename, strmethod, output_path, format='.pdf', folder=''):
    """
    Generate the full file path for saving a plot.

    Args:
        filename (str): The base file name.
        strmethod (str): A string to include in the file name.
        output_path (str): The directory to save the file.
        format (str): The file format (default: '.pdf').
        folder (str): A subfolder within the output directory (optional).

    Returns:
        str: The full file path for the plot.
    """
    # Remove '.csv' extension if it exists
    if filename.endswith('.csv'):
        filename = filename[:-4]
    
    # Replace '.' in the filename to avoid issues
    filename = filename.replace('.', 'p')
    
    # Include the folder inside the output path if specified
    if folder:
    ...
    output = os.path.join(output_path, f"{strmethod}_{filename}{format}")
    return output
...
def name_folder(dir, strmethod):
    """
    Create and return a directory path based on the given inputs.

    Args:
        dir (str): The base directory.
        strmethod (str): A string to include in the folder name.
        filename (str, optional): A filename to include in the folder name.

    Returns:
        str: The complete path to the created directory.
    """
    ...
    folder_name = f"{strmethod}_fit_{timestamp}"
    ...
    try:
        if params:
            if func == ff.cavity_inverse:
                ...
                if Qi < 0 and Method.name != 'DCM REFLECTION':
                    print(
