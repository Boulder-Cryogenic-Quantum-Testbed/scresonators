...
def PlotFit(x,
    ...
    try:
            ...
            if func == ff.cavity_inverse:
                if params[0] < 0:
                    print("Qi is less than zero. Please make sure data is of"
                          " correct format: decibels (log10*20 version), and "
                ...
                if 1 / np.real(1 / Qc) < 0:
                    ...
                    textstr = 'Q = ' + '%s' % float('{0:.10g}'.format(Q)) + r"$\pm$" + '%s' % float(
                        ...
                        '{0:.2g}'.format(conf_array[4])) + ' radians' + \
                              '\n' + r'$f_c$ = ' + '%s' % float('{0:.7g}'.format(f_c / fcscale)) + r"$\pm$" + '%s' % float(
                    ...
                    for val in reports:
                        if val == r'$f_c$':
                            ...
                            continue # hack so indices of report and conf_array are the same
                        else:
                            vscale = 1.
        ...
        print(f">{e}")
        raise RuntimeError(">Error when trying to write parameters on plot")
    ...
    try:
        ...
        print(f">{e}")
        raise RuntimeError(">Error when trying to create metadata file")
