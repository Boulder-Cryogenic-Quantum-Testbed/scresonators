...
def from_file(filepath, data_column=None, fscale=1):
    ...
    if extension.startswith('.s') and extension.endswith('p'):
        try:
            ...
            raise ValueError(f'Data file: {filepath} could not be found/read')
        ...
        try:
            ...
            raise RuntimeError(f'Exception: **{e}** encountered when attempting to load data file as .txt/.csv')
        ...
        raise RuntimeError(f'Please use .s2p, .s1p, .txt, or .csv')
...
def data_parse(s_col, line, frequency_units, data_format, file, options):
    ...
    data_rows = [3, 4]
    if len(row) == 0:
        raise RuntimeError("Data not found in file.")
    ...
    if data_format == "db":
        ...
        raise RuntimeError("Data type in file not supported. Please use DB, MA, or RI.")
