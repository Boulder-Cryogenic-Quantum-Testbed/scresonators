...
if __name__ == '__main__':
    ...
    powers_in_high = np.linspace(-5, -25, 5)
    powers_in_low = np.linspace(-30, -90, 31)
    powers_in = np.hstack((powers_in_high,
                            powers_in_low,
                            [-91, -93, -95, -100, -105]))
    powers_in = np.linspace(-10, -86, 20)
    print(powers_in)
    tstr = f'{temperature*1e3:.0f}mK'
    samp_name = 'NYU_AL_INP'

    # Set the filename strings
    filenames_in = [f'{samp_name}_221019_6_6p588GHz_{int(p)}dB_{tstr}.csv' 
                for p in powers_in]
    # filenames_in = [f'{samp_name}_220930_6_6p112GHz_{int(p)}dB_{tstr}.csv' 
    #             for p in powers_in]

    Run the power sweep
    power_sweep_fit_drv(sample_name=samp_name+'_5p704GHz',
                        ...
                        plot_twinx=False, plot_fit=True)
