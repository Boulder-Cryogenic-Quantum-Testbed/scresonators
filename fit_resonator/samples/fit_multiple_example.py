...
def fit_multiple(names, dir, norm=10, pickle=False):
    ...
    for i in names:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to initialize method, please change parameters. ERROR: {e}")
        ...
        if pickle:
            output = open(dir + filename + "_serialized", 'wb')
            pickle.dump(resonator, output)
            output.close()
...
def resonators_from_pickle(dir, names):
    resonators = []
    for name in names:
        output = open(dir + name + "_serialized", 'rb')
        resonators.append(pickle.load(output))
        output.close()
    return resonators

fit_multiple(names, dir, pickle = True)
